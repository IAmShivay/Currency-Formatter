...
export const formatCurrency = (amount: number, options: CurrencyFormatterOptions): string => {
  const { numberingSystem, currency } = options;
  const formattedNumber =
    numberingSystem === 'indian' ? formatNumberIndian(amount) : formatNumber(amount, 1);
